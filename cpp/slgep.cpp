...
public:
	...
	void select_from_local_and_remote_populations(int shuffling_method, int selection_method, int aggregation_method)
	{
		...
		else if (shuffling_method == 1)
		{ // ** Select by the most fit
		...
		case 3: // ** Replace worst with remote worst if average is better
			for (int i = 0; i < int(POPSIZE * 0.1); i++)
			{
				...
				if (average_fitness < population[i].fitness)
				{ // The remote chromosome is better
					population[i] = remote_population[i];
					this->compute_chromosome_fitness(&population[i]);
				}
			}
...
private:
	...
	void its_a_new_generation()
	{
		...
		for (int i = 0; i < POPSIZE; i++)
		{
			...
			{
				r1 = rand() % POPSIZE;
			} while (r1 == i); // Dont mutate the vector with itself
			do
			{
				r2 = rand() % POPSIZE;
			} while (r2 == r1 || r2 == i); // Need a third vector
			...
			for (int j = 0; j < NVARS; j++)
			{
				if (randval(0, 1) < CR || k == j)
				{
					...
					if (((int)best_chromosome.gene[j]) != ((int)population[i].gene[j]))
						dd1 = 1;
					if (((int)population[r1].gene[j]) != ((int)population[r2].gene[j]))
						dd2 = 1;
	...
	void decode_gene(int gene[NVARS])
	{
		...
		for (int g = 0; g < GSIZE; g++)
		{
			...
			if (sub_root[g]->value < L_terminal)
			{ // Note that L_input > L_terminal;
				do
				{ // Find an op type item
					do
					{
						...
						if (op >= i)
							break;
					} while (sub_comp[g][op].value >= L_terminal);
					if (op >= i)
						break;
					// Set its left and right;
	...
	void compute_rule(const struct LINK_COMP *node)
	{
		...
		else
		{
			...
			case 6: // exp
				if (t1 < 20)
					current_value = exp(t1);
				else
					current_value = exp(20.);
				break;
			case 7: // log
				if (fabs(t1) < 1e-20)
					current_value = 0;
				else
					current_value = log(fabs(t1));
				break;
	...
	void compute_sub_rule(const struct LINK_COMP *node)
	{
		...
		else
		{
			...
			if (node->value < 4)
			{ // note that the first 4 functions have 2 children
			...
			case 3: // /
				...
				break;
			case 4: // sin
				sub_current_value = sin(t1);
				break;
			case 5: // cos
				sub_current_value = cos(t1);
				break;
			case 6: // exp
				if (t1 < 20)
					sub_current_value = exp(t1);
				else
					sub_current_value = exp(20.);
				break;
			case 7: // log
				if (fabs(t1) < 1e-20)
					sub_current_value = 0;
				else
					sub_current_value = log(fabs(t1));
				break;
			default:
		...
		for (i = 0; i < POPSIZE; i++)
		{
			for (j = 0; j < H; j++)
			{
				if (population[i].gene[j] < L_terminal)
					FQ++;
				else if (population[i].gene[j] >= L_terminal)
					CC++;
			}
		}
